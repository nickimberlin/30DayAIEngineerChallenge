...
- **Tokens (front matter)**: `{spacing.xxs}` 4px · `{spacing.xs}` 8px · `{spacing.sm}` 12px · `{spacing.md}` 16px · `{spacing.lg}` 24px · `{spacing.xl}` 32px · `{spacing.xxl}` 48px · `{spacing.xxxl}` 64px · `{spacing.section}` 128px.
- Card interior padding: `{spacing.xl}` 32px on all feature cards, product cards, and panels; `{spacing.xxl}` 48px on hero cards and CTA banners.
- Button padding: 12px vertical · 16px horizontal — Carbon spec.
- Form input padding: 11px vertical · 16px horizontal.
- Inter-card gap: `{spacing.lg}` 24px minimum between grouped cards and panels.
- Section-to-section separation: `{spacing.xxl}` 48px minimum vertical padding between adjacent sections.
...
Carbon uses precise alignment to a 4-pixel grid as its whitespace system. Sections separate via thin gray rows (`{colors.surface-1}`) combined with generous vertical padding. Every card, panel, and section interior uses `{spacing.xl}` 32px padding — never less — so that content sits comfortably within its container without feeling cramped. Inter-card gaps use `{spacing.lg}` 24px minimum. Section vertical margins land at `{spacing.xxl}` 48px or larger. The extra breathing room makes dense information easier to scan at a glance — content stays rich but no longer crowded.

### Spacing in Practice

| Location | Token | Value | Rule |
|---|---|---|---|
| Card interior padding (feature, product, analysis) | `{spacing.xl}` | 32px | Every card body uses 32px interior padding |
| Hero card / CTA banner interior | `{spacing.xxl}` | 48px | Larger containers get more breathing room |
| Resource tile / small panel interior | `{spacing.lg}` | 24px | Smaller cards use 24px minimum |
| Inter-card gap (grid) | `{spacing.lg}` | 24px minimum | Between adjacent cards in a grid; 32px preferred |
| Section-to-section vertical separation | `{spacing.xxl}` | 48px minimum | Vertical padding between adjacent sections on alternating surfaces |
| Section header to content gap | `{spacing.lg}` | 24px | Between a section heading and the first card row |
| Results panel: ScoreCard to detail grid | `{spacing.xl}` | 32px | Gap between overview score and detailed breakdowns |
| Input group: stacked cards | `{spacing.lg}` | 24px | Between stacked cards in a single column |
| Page edge to content | `{spacing.xl}` | 32px | Horizontal gutter at desktop; collapses to 16px on mobile |
| Feature row bottom margin | `{spacing.xxl}` | 48px | After the last card row before the next section band |

Implement using the utility classes in `index.scss`: `.r-section-top` (96px), `.r-section-bottom` (64px), `.r-section-side` (32px horizontal). These handle the outer section rhythm. Inner card padding and grid gaps should be set via inline styles consistently at the values above.
...
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body}`, rounded `{rounded.none}`, padding 32px. Stroked with 1px `{colors.hairline}`.
...
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-sm}`, rounded `{rounded.none}`, padding 24px.
